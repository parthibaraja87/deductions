 ...
 */
function exportFlexChange(syncRule, transferParameters, ids, breadcrumbs, title, configObject, key) {
...
function exportManifestChange(ids, title, syncRule, transferParameters, key, configObject, propertyDefinition, nextDefinition) {
    ...
    const { targetAnnotationEncoded, targetAnnotation, custom } = determineTargetAnnotation(ids, title);
 ...
 */
function evaluateExportRule(transferParameters, breadcrumbs, key, configObject, ids, title, propertyDefinition, nextDefinition) {
    const syncRule = common_1.getReflectMetadata(configObject, key);
    if (syncRule) {
        if (syncRule.flex) {
            exportFlexChange(syncRule, transferParameters, ids, breadcrumbs, title, configObject, key);
        }
        else if (syncRule.manifest) {
            exportManifestChange(ids, title, syncRule, transferParameters, key, configObject, propertyDefinition, nextDefinition);
        }
    }
}
...
function transferManifestEntriesAndFlexChange(transferParameters, configObject, parentIds, stableIdParts, breadcrumbs, schemaDefinition, title) {
    ...
    if (schemaDefinition && schemaDefinition['properties']) {
        for (const key in schemaDefinition['properties']) {
            ...
            evaluateExportRule(transferParameters, breadcrumbs, key, configObject, ids, title, propertyDefinition, nextDefinition);
